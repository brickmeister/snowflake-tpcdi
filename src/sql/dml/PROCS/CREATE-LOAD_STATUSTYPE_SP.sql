CREATE OR REPLACE PROCEDURE TPCDI_STG.PUBLIC.LOAD_STATUSTYPE_SP(scale float)
  ...
  stmt = snowflake.createStatement(
      {sqlText: "COPY INTO TPCDI_STG.PUBLIC.STATUSTYPE_STG FROM @TPCDI_FILES//tpcdi/sf=" + tpcdi_scale + "/Batch1/StatusType.txt FILE_FORMAT = (FORMAT_NAME = 'TXT_PIPE') ON_ERROR = SKIP_FILE"}
    );
  rs = stmt.execute();
  rs.next();
  // Stop task
