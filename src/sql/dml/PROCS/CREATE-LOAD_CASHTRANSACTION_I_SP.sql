CREATE OR REPLACE PROCEDURE TPCDI_STG.PUBLIC.LOAD_CASHTRANSACTION_I_SP(scale float,batches float,wait float)
	...
	while (batch_counter <= BATCHES)	
	{
		var incrm_stmt = snowflake.createStatement(
			{sqlText: "COPY INTO TPCDI_STG.PUBLIC.CASHTRANSACTION_STG FROM @TPCDI_FILES//tpcdi/sf=" + tpcdi_scale + "/Batch" + batch_counter + "/CashTransaction FILE_FORMAT = (FORMAT_NAME = 'TXT_PIPE') ON_ERROR = SKIP_FILE"}
			);
		incrm_stmt.execute();
		// insert wait here
